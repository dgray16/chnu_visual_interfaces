    ...
    private VBox appendGenerateButton() {
        ...
        button.setOnAction(event -> {
            ...
            if ( matrix == null ) {
                ...
                if ( rows > Util.MAX_ROWS || columns > Util.MAX_COLUMNS ) {
                    generateErrorAlert("No more than " + Util.MAX_ROWS + " rows and " + Util.MAX_COLUMNS + " columns");
    ...
    private VBox appendClearButton() {
        ...
        button.setOnAction(event -> {
            ...
            if ( resultMatrix != null ) {
                matrixResultPane.getChildren().remove(resultMatrix.getParent());
                resultMatrix = null;
            }
    ...
    private VBox appendProcessButton() {
        ...
        button.setOnAction(event -> {
            if ( !matrix.getChildren().isEmpty() ) {
                if ( isSDI ) {
                    /* TODO intead of new pane, put result matrix lower than basic matrix */
                    matrixResultPane = matrixMainPane;
    ...
    private void createRowValuesLabels(boolean isMainMatrixPane) {
        if ( isMainMatrixPane ) {
            ...
            if ( !rowValuesListLabelsOnResultMatrixPane.isEmpty() ) {
                ...
                int y = Double.valueOf(resultMatrix.getParent().getLayoutY()).intValue() + 11;
